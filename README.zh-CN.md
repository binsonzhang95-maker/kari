...
可自托管的单租户 **团队文件共享 + 远程终端**。

把团队指向同一台共享主机:项目文件通过 [Syncthing](https://syncthing.net/)
双向同步,任何成员都能在该主机上打开真实的交互式 shell 或执行命令 —— 全部由
一个共享密钥把关,走端到端加密连接。桌面应用、Web 控制台、命令行都连同一台 server。

这是开源的单租户内核:**无注册中心、无计费、无多租户密钥库、无云、无 LLM 代理、
无容器**。一个密钥、一个同步目录、一个自带 Web 控制台的 server 二进制。
...
- **双向文件同步**,由 Syncthing 承载 —— server 把 Syncthing 作为私有、关闭发现的
  sidecar 运行,按需为客户端设备配对。
- **远程交互式终端** —— POSIX 上是真 PTY,Windows 上是 ConPTY。
- **远程命令执行**,输出流式返回。
- **会话历史** —— 展示某主机的 `~/.claude` / `~/.codex` 会话。
- **MCP local-exec 桥** 给会话里运行的 AI CLI 用。
